...
def generator_loss(disc_generated_output, gen_output_coarse, gen_output_refine, target, mask):
    ...
    total_loss = (
        VALID_LOSS_WEIGHT * valid_l1_loss
        + HOLE_LOSS_WEIGHT * hole_l1_loss
        + EDGE_LOSS_WEIGHT * edge_loss
        + GAN_LOSS_WEIGHT * gan_loss
        + PERCEPTUAL_LOSS_OUT_WEIGHT * perceptual_loss_out
        + PERCEPTUAL_LOSS_COMP_WEIGHT * perceptual_loss_comp
    )
...
def train_step(original_image, masked_image, mask, epoch):
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        total_loss, valid_l1_loss, hole_l1_loss, edge_loss, gan_loss, pl_out, pl_comp = generator_loss(
            disc_generated_output, prediction_coarse, prediction_refine, original_image, mask
        )
...
if __name__ == "__main__":
    ...
    selected_layers = ["block3_conv4", "block4_conv4", "block5_conv3", "block5_conv4"]
